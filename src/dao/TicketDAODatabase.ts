...
export class TicketDAODatabase implements TicketDAO {
  ...
  async update(ticket: any): Promise<void> {
    ...
    await connection.query(
      "update cifer.ticket set status = $1, assignee_id = $2, end_date = $3, duration = $4 where ticket_id = $5",
      [
        ticket.status,
        ticket.assigneeId,
        ticket.endDate,
  ...
  async updateStatus(ticketId: string, status: string): Promise<void> {
    ...
    await connection.query(
      "update cifer.ticket set status = $1 where ticket_id = $2",
      [status, ticketId]
    );
