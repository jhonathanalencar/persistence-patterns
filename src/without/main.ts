...
app.post("/tickets", async function (req, res) {
  ...
  await connection.query(
    "insert into cifer.ticket (ticket_id, requester_id, content, start_date, status) values ($1, $2, $3, $4, $5)",
...
app.post("/tickets/:ticketId/assign", async function (req, res) {
  ...
  const [ticketData] = await connection.query(
    "select * from cifer.ticket where ticket_id = $1",
    [req.params.ticketId]
  );
  ...
  await connection.query(
    "update cifer.ticket set status = $1, assignee_id = $2 where ticket_id = $3",
...
app.post("/tickets/:ticketId/close", async function (req, res) {
  ...
  if (ticketData.status === "open")
    throw new Error("The ticket is not assigned");
  ...
  await connection.query(
    "update cifer.ticket set status = $1, end_date = $2, duration = $3 where ticket_id = $4",
    [req.body.status, endDate, duration, req.params.ticketId]
  );
